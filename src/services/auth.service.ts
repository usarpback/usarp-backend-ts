...
import { AuthenticatedUser } from "@/types/auth.type.js";
...
export async function getUserLockedStatus(user: User): Promise<UserStatus> {
  ...
  if (user.bloqueadoAte && isAfter(now, new Date(user.bloqueadoAte))) {
    ...
    return {
      status: "reset",
      message:
        "A conta estava bloqueada, mas o tempo de bloqueio expirou. O acesso foi liberado.",
    };
  }
...
export async function getUserAttemptsStatus(
  user: User,
  ...
  return reachedMaxAttempts
    ? {
        ...status,
        message:
          "Conta bloqueada por 10 minutos devido a múltiplas tentativas incorretas.",
      }
    : {
        ...status,
        message: `Senha inválida. Você ainda tem ${3 - attempts} tentativa(s).`,
      };
}
