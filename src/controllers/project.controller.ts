...
export async function getUserProjects(request: FastifyRequest, reply: FastifyReply) {
  const userId = request.user?.id;

  const projects = await projectRepository.findAllByCreatorId(userId!);
  if (!projects) return reply.status(400).send('Não foi possível buscar os projetos');
...
export async function createProject(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!project) return reply.status(400).send('Não foi possível criar o projeto');
